 ...
 *
 * This MUST run before Playwright evaluates its config because Playwright loads
 * the config module in every worker process. If the port were chosen inside the
 * config, each worker would race and pick a different port than the one the
 * webServer is bound to. Called automatically by `pnpm test:e2e`.
 */
