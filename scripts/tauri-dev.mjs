 ...
 * ANSI colour codes, hostnames other than 127.0.0.1, and output split across
...
function detectVitePort(child, timeoutMs = 30_000) {
  return new Promise((resolve, reject) => {
    ...
    // If Vite exits before printing the port banner, reject immediately so the
    // caller gets a useful error rather than waiting for the timeout.
...
export async function runTauriDev(extraTauriArgs = []) {
  // Remove any stale merge file from a previous run; ignore missing file, but
  // rethrow real filesystem errors (permissions, etc.).
  rmSync(mergePath, { force: true });

  let viteShuttingDown = false;
  // Becomes true once Vite is fully serving and Tauri has launched. The exit
  // handler below only escalates crashes that happen AFTER startup.
  ...
  // Point Tauri at the port Vite actually bound, and null out beforeDevCommand
  // so Tauri does not spawn a second Vite of its own.
  const merge = {
    build: {
      devUrl,
  ...
  const exitCode = await new Promise((resolve, reject) => {
    ...
    tauriChild.on("exit", (code) => resolve(code ?? 0));
  });
